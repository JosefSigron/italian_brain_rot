...
4. Set up your ElevenLabs API key:
   ...
   ```
   (On Windows, use `set ELEVENLABS_API_KEY=your_api_key_here`)
   ...
   For more information visit: [ElevenLabs](https://elevenlabs.io/)

5. Set up YouTube API credentials (If you are not planning on uploading to youtube, you may skip this step):
...
This project now uses ElevenLabs for high-quality text-to-speech generation instead of OpenAI's TTS. ElevenLabs provides:

- **Better Quality**: More natural-sounding voices with better pronunciation
- **Multilingual Support**: Excellent Italian language support
- **Voice Variety**: Multiple voice options to choose from
- **Customization**: Adjustable voice settings for stability and style

### Voice Settings

The default voice settings are optimized for Italian narration:
- **Stability**: 0.5 (balanced between consistency and expressiveness)
- **Similarity Boost**: 0.75 (maintains voice characteristics)
- **Style**: 0.0 (neutral storytelling style)
- **Speaker Boost**: Enabled (enhances clarity)

### Changing Voices

To use a different voice:
1. Run `python src/setup_elevenlabs.py` to see available voices
2. Copy the voice ID you want to use
3. Update the `voice_id` parameter in `src/generate_speech.py`
